...
def coleta_ga4(desde, ate):
    # GA4 pela Analytics Data API (nativa, grátis). "conversoes" = transactions (compras).
    ...
    log(f"GA4 OK (API nativa): {len(paginas)} linhas de página")
    ...
    return {
        ...
        "fontes": {
            ...
            "google": {"estado": "ok" if google_diario else "pendente",
                       "motivo": None if google_diario else "Aguardando developer token da API do Google Ads"},
            "ga4": {"estado": "ok" if ga4_paginas else "pendente",
                    "motivo": None if ga4_paginas else "GA4 sem dados no período"},
